...
Write-Output "[$((Get-Date).TimeofDay)] INFO: Finding All The Gpos In $Domainname" -ForegroundColor Yellow
...
# Backing Up Gpo
Write-Output "[$((Get-Date).TimeofDay)] INFO: Backing Up All The Gpos In $Domainname" 
Set-Location -Path $path
Foreach ($Allgpos In $Allgposindomain) {
    Write-Output "[$((Get-Date).TimeofDay)] INFO: Backing Up ("$Allgpos.Displayname")"
...
Write-Output "[$((Get-Date).TimeofDay)] INFO: $($GPOsCount.Count) GPOs have been backed up to $Gpo" -ForegroundColor Yellow
